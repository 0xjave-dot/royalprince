...
export default function Home() {
  ...
  return (
    <div className="flex-1 flex flex-col bg-white animate-fade-up-enter pb-10">
      {/* Top Header Section */}
      <div className="sticky top-0 z-[110] flex md:hidden shrink-0 items-center gap-1.5 px-4 sm:px-5 h-8 bg-white/90 backdrop-blur-xl border-b border-[#e5e5e5] shadow-[0_4px_18px_rgba(0,0,0,0.04)]">
        <div className="flex items-center gap-1 flex-1 min-w-0 select-none">
          <img
            src={brandLogoUrl}
            alt={`${brandName} logo`}
            ...
            referrerPolicy="no-referrer"
          />
          <h1 className="font-display font-black text-[16px] tracking-tight text-dark uppercase leading-none">
            {brandName}
          </h1>
        </div>
        ...
        <div
          ...
          className="relative cursor-pointer w-7 h-7 flex items-center justify-center bg-gray rounded-full hover:bg-gray3/30"
        >
          <ShoppingBag className="w-4 h-4 text-dark" strokeWidth={2} />
          {itemCount > 0 && (
      ...
      <div className="px-4 sm:px-5 space-y-2.5">
        <ScrollReveal className="mt-2.5">
          <DailyPromoCarousel />
        </ScrollReveal>

        {/* New arrivals section */}
        <div className="sticky top-8 z-[100] -mx-4 sm:-mx-5 px-4 sm:px-5 py-2 bg-white/92 backdrop-blur-xl border-b border-[#e5e5e5] md:static md:mx-0 md:px-0 md:py-0 md:bg-transparent md:backdrop-blur-0 md:border-0">
        ...
        <ScrollReveal delay={0.08} className="py-1 flex justify-center w-full">
          <div className="mx-auto flex w-full max-w-md flex-nowrap justify-center gap-0.5 overflow-x-auto no-scrollbar pb-1 sm:gap-1.5">
            {isLoading ? (
              Array.from({ length: 4 }).map((_, idx) => (
                <div key={idx} className="h-[3rem] w-[3rem] sm:h-[3.5rem] sm:w-[3.5rem] shrink-0 rounded-[22px] bg-[#f8f8f8] animate-pulse" />
              ))
            ) : (
              categories.map((cat) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      ...
                      className={`relative w-[2.75rem] h-[2.75rem] sm:w-[3.25rem] sm:h-[3.25rem] aspect-square rounded-[22px] border flex items-center justify-center transition-all duration-300 shadow-subtle active:scale-95 overflow-hidden ${
                        isActive
                    ...
                    >
                      ...
                      <div className="relative transition-transform duration-300 group-hover:scale-110">
                        {getCategoryIcon(cat.slug, categories, "w-4.5 h-4.5 sm:w-5.5 sm:h-5.5")}
                      </div>
                    </div>
                  </div>
                );
